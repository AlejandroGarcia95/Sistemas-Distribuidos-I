...
int main(){
	...
	dshm_init(mom, "parent_shm", 0);
	dshm_init(mom, "child_shm", 0);
	
	dsem_init(mom, "child_can_read", 0);
	dsem_init(mom, "parent_can_read", 0);
	dsem_init(mom, "child_finished", 0);
	dsem_init(mom, "parent_finished", 0);
	...
	if (pid == 0) {
		char *const parm_list[] = {"./museum_door", NULL};
		execv("./museum_door", parm_list);
	}
	...
	// Parent loop
	for(int i = 0; i < 6; i++) {
		int parent_value = 2 * i + 1;
		dshm_write(mom, "parent_shm", parent_value);
		dsem_signal(mom, "child_can_read");
		dsem_wait(mom, "parent_can_read");
		int child_value;
		dshm_read(mom, "child_shm", &child_value);
		printf("Museum-Turn %d: (Parent: %d, Child:%d)\n", i + 1, parent_value, child_value);
		dsem_signal(mom, "parent_finished");
		dsem_wait(mom, "child_finished");
	}
...
void launch_guide(ap_t* ap_people, pid_t* children_pids){
	...
	// shm and sem of tour
	int shmid = shm_create("museum_guide.c", sizeof(int));
	int* aux = (int*) shm_attach(shmid);
	*aux = 0;
	shm_detach(aux);
	int semid = sem_create(1, "museum_guide.c");
	sem_init(semid, 0, 0);
	int semid_vacancy = sem_create(1, "museum_person.c");
...
int main(int argc, char* argv[]) {
	int semid = sem_create(1, "museum_main.c");
	sem_init(semid, 0, 1);
	
	int shmid = shm_create("museum_main.c", sizeof(int));
