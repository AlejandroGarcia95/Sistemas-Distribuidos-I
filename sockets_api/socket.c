...
void socket_bind(socket_t* self, char* hostname, char* port){
	...
	if(err){
		printf("%d: Error in getaddrinfo: %d\n", getpid(), err);
        exit(-1);
	}
	...
	if(err < 0) {
		freeaddrinfo(self->ai);
		self->ai = NULL;
		printf("%d: Error binding socket: %d\n", getpid(), errno);
        exit(-1);
	}
}
...
void socket_listen(socket_t* self, int backlog) {
	if(self->type != SOCK_PASSIVE){
		printf("%d: Error in socket_listen, received socket is not SOCK_PASSIVE!\n", getpid());
        exit(-1);
	}

	if (!backlog) backlog = BACKLOG_DEFAULT;
	if(listen(self->sock, backlog) < 0){
		printf("%d: Error in pasive socket listening: %d\n", getpid(), errno);
        exit(-1);
	}
}
...
socket_t* socket_accept(socket_t* self) {
	...
	if (new_sock->sock < 0) {
		free(new_sock);
		new_sock = NULL;
		printf("%d: Error in socket accepting: %d\n", getpid(), errno); 
	}
	return new_sock;
}
...
void socket_conect(socket_t* self, char* server_ip, char* server_port) {
	if(self->type != SOCK_ACTIVE){
		printf("%d: Error in socket_conect, received socket is not SOCK_ACTIVE!\n", getpid());
        exit(-1);
	}

	struct addrinfo* ai;
	int err = init_addrinfo(&ai, server_ip, server_port);
	if(err){
		printf("%d: Error in getaddrinfo: %d\n", getpid(), err);
        exit(-1);
	}
	...
	if(err < 0){
		printf("%d: Error connecting active socket: %d\n", getpid(), errno);
        exit(-1);
	}
